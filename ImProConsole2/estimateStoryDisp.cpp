...
{
    int nPoint = objPoints.rows;
    objPoints.copyTo(newObjPoints);

    for (int iPoint = 0; iPoint < nPoint; iPoint++)
    {
        double ux = disp.at<double>(0, 0);
        double uy = disp.at<double>(1, 0);
        double th = disp.at<double>(2, 0) * 3.14159265358979324 / 180.0;
        cv::Point3d oldPoint = objPoints.at<cv::Point3d>(iPoint, 0);
        cv::Point3d newPoint;
        center.x += ux;
        center.y += uy;
        newPoint.x = (oldPoint.x - center.x) * cos(th) - (oldPoint.y - center.y) * sin(th) + center.x + ux;
        newPoint.y = (oldPoint.x - center.x) * sin(th) + (oldPoint.y - center.y) * cos(th) + center.y + uy;
        newPoint.z = oldPoint.z;
        newObjPoints.at<cv::Point3d>(iPoint, 0) = newPoint;
    }
    return 0;
}
	...
	cv::Mat & newTvec)  // updated tvec (3, 1, CV_64F)
{
	...
	// confirm type and dimension
	rvec.convertTo(rvec, CV_64F);
	tvec.convertTo(tvec, CV_64F);
	camr.convertTo(camr, CV_64F);
	if (rvec.rows < rvec.cols) rvec = rvec.t();
	if (tvec.rows < tvec.cols) tvec = tvec.t();
	if (camr.rows < camr.cols) camr = camr.t();
	rvec.copyTo(newRvec);
	tvec.copyTo(newTvec); 

	// variable declaration
	cv::Mat r3(3, 3, CV_64F), r4(4, 4, CV_64F), camRotR3(3, 3, CV_64F); 

	// rvec tvec --> r3 and r4
	r4 = cv::Mat::eye(4, 4, CV_64F);
	r3 = cv::Mat::eye(3, 3, CV_64F);
	...
	// inverse r4 (to acquire camera axes and position)
	r4 = r4.inv();
	r4(cv::Rect(0, 0, 3, 3)).copyTo(r3);
	if (debug) cout << "inv of r4:\n" << r4 << endl;

	// get camera rotation matrix (3x3)
	cv::Rodrigues(camr, camRotR3);
	if (debug) cout << "cam r3:\n" << camRotR3 << endl;

	// rotate camera by changing r3 then update r4
	r3 = r3.clone() * camRotR3;
	r3.copyTo(r4(cv::Rect(0, 0, 3, 3)));
	if (debug) cout << "new inv of r4:\n" << r4 << endl;

	// inverse r4 back, then convert to new rvec and new tvec
	r4 = r4.inv();
	r4(cv::Rect(0, 0, 3, 3)).copyTo(r3);
	r4(cv::Rect(3, 0, 1, 3)).copyTo(newTvec);
	cv::Rodrigues(r3, newRvec);
	if (debug) cout << "new r4:\n" << r4 << endl;
	if (debug) cout << "new rvec:\n" << newRvec << endl;
	if (debug) cout << "new tvec:\n" << newTvec << endl;
}
...
*/
vector<double> costFuncStoryDisp(
	...
	cv::Mat refObjPointsMat,  // object points of reference points(supposed to be fixed on lower story), cv::Mat(N1, 1, CV_64FC3)
	cv::Mat trkObjPointsMat,  // object points of tracking points(moving points on upper story), cv::Mat(N1, 1, CV_64FC3)
	cv::Mat newRefImgPointsMat,  // new reference image points(on lower story), cv::Mat(N1, 1, CV_32FC2)
	cv::Mat newTrkImgPointsMat,  // new traking image points(on upper story), cv::Mat(N2, 1, CV_32FC2)
	...
	cv::Mat & y // Output : resicual vector.cv::Mat(2 * # of all points, 1, CV_64F)
	)
{
	cv::Mat newDisp(3, 1, CV_64F); 
	cv::Mat camRot(3, 1, CV_64F);
	cv::Mat newTvec, newRvec, r4, r3, camRotR3(3, 3, CV_64F); 
	cv::Mat newTrkObjPoints, newProjRefImgPoints, newProjTrkImgPoints;
	...
	cv::projectPoints(refObjPointsMat, newRvec, newTvec, cmat, dvec, newProjRefImgPoints);
	cv::projectPoints(newTrkObjPoints, newRvec, newTvec, cmat, dvec, newProjTrkImgPoints);
	newProjRefImgPoints.convertTo(newProjRefImgPoints, newRefImgPointsMat.type());
	newProjTrkImgPoints.convertTo(newProjTrkImgPoints, newTrkImgPointsMat.type());
	...
	if (newProjRefImgPoints.cols > newProjRefImgPoints.rows) newProjRefImgPoints = newProjRefImgPoints.t();
	if (newProjTrkImgPoints.cols > newProjTrkImgPoints.rows) newProjTrkImgPoints = newProjTrkImgPoints.t();
	// find residual 
	refProjError = newProjRefImgPoints - newRefImgPointsMat;
	trkProjError = newProjTrkImgPoints - newTrkImgPointsMat;
	// residual
...
/*!
	\brief Function estimateStoryDispWithoutReturningZero() estimates story displacement function of story displacement 
	\param cmat					camera matrix of intrinsic parameters, cv::Mat(3, 3, CV_32F)
	\param dvec					camera distortion vector of intrinsic parameters, cv::Mat(1, 4 or 5 or 8, CV_32F)
	\param rvec					rotational vector of camera extrinsic parameters, cv::Mat(3, 1, CV_64F)
	\param tvec					translational vector of camera extrinsic parameters, cv::Mat(3, 1, CV_64F)
...
*/
// Note:In this function estimateStoryDispWithoutReturningZero(), 
...
int estimateStoryDispWithoutReturningZero(
	cv::Mat cmat, // camera matrix of intrinsic parameters, cv::Mat(3, 3, CV_32F)
	cv::Mat dvec, // camera distortion vector of intrinsic parameters, cv::Mat(1, 4 or 5 or 8, CV_32F)
	cv::Mat rvec, // rotational vector of camera extrinsic parameters, cv::Mat(3, 1, CV_64F)
	cv::Mat tvec, // translational vector of camera extrinsic parameters, cv::Mat(3, 1, CV_64F)
...
)
{
	...
	// get y0
	costFuncStoryDisp(cmat, dvec, rvec, tvec, 
		refObjPointsMat,
		trkObjPointsMat,
		newRefImgPointsMat,
		newTrkImgPointsMat, 
...
/*!
	\brief Function estimateStoryDisp() estimates story displacement function of story displacement
	\param cmat					camera matrix of intrinsic parameters, cv::Mat(3, 3, CV_32F)
	\param dvec					camera distortion vector of intrinsic parameters, cv::Mat(1, 4 or 5 or 8, CV_32F)
	\param rvec					rotational vector of camera extrinsic parameters, cv::Mat(3, 1, CV_64F)
	\param tvec					translational vector of camera extrinsic parameters, cv::Mat(3, 1, CV_64F)
...
)
{
	...
	estimateStoryDispWithoutReturningZero(cmat, dvec, rvec, tvec,
...
int test_calcStoryDispNewObjPoints()
{
	bool debug = false; 
...
int test_estimateStoryDisp() 
{
	...
	estimateStoryDisp(cmat, dvec, rvec, tvec,
		refImgPoints,
		trkImgPoints,
		refObjPoints,
		trkObjPoints,
